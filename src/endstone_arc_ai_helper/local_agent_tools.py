    ...
    tools: List[Dict[str, Any]] = [
        ...
        _fn(
            ...
            (
                ...
                "权限受三档限制：助手仅 tp/effect/spawnpoint 等（不可 give）；"
                "管理员大部分指令（含 give）；代理服主全部。"
                "禁止滥用 stop/kill；敏感权限指令仅代理服主。"
                + (
                    " 神灵模式已启用：普通玩家/助手身份的个人神恩（effect/tp 等，物品须 item_id）"
            ...
            ),
            {
                "command": {
                    "type": "string",
                    "description": "不含开头斜杠的控制台指令，例如 effect Steve night_vision 30 0 true",
                }
            },
            ["command"],
        ),
