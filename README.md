...
- **上下文管理**
  - 走 **AstrBot** 时：中枢用已绑定的 **QQ 号**当用户 ID（没绑定就用 **XUID**），不传群号；改名或跨 QQ 群仍能对上同一个人。本插件只送当前这句话、`system_prompt.txt` 和玩家 XUID。执行指令时在**这条消息来源的那台服**上跑。中枢还会给模型挂上查在线、查 TPS、执行指令等工具。
