...
[![Version](https://img.shields.io/badge/version-v2.0.0-blue)](https://github.com/ARC-Minecraft/EndstoneMC-ARC-AI-Helper)
...
一个为 Endstone 服务器提供 **AI 聊天助手** 功能的插件（当前 **v2.0.0**）。支持：
...
- **上下文管理**
  - 走 **AstrBot** 时：中枢用已绑定的 **QQ 号**当用户 ID（没绑定就用 **XUID**），不传群号；改名或跨 QQ 群仍能对上同一个人。本插件只送当前这句话、`system_prompt.txt`、玩家 XUID，以及 **AI 权限级别**（`permission_level`）。执行指令时在**这条消息来源的那台服**上跑。中枢还会给模型挂上查在线、查 TPS、执行指令，以及（本服有监狱插件时）一键入狱 / 释放 / 查看在押、（本服有弧光核心时）银行 / 领地 / 传送 / 天眼等工具。
...
在 `chat_config.json` 里把 `"default_permission_level": "admin"` 即可让全服默认管理员级别（你的服可直接这样设）。也可用 `"permission_overrides": { "玩家名或XUID": "admin" }` 单独指定。
...
- **default_permission_level**（v2.0.0）：默认 AI 权限，可选 `assistant` / `admin` / `proxy_owner`（或中文：助手 / 管理员 / 代理服主）。
- **op_maps_to_admin**（v2.0.0）：游戏 OP 是否自动升到管理员级别，默认 `true`。
- **permission_overrides**（v2.0.0）：按玩家名或 XUID 覆盖权限，例如 `{ "Steve": "admin", "25354…": "proxy_owner" }`。
...
- **权限节点**
  ...
  - `arc_ai_helper.permission.assistant`：助手级别（默认 `True`）。
  - `arc_ai_helper.permission.admin`：管理员级别（默认 `False`）。
  - `arc_ai_helper.permission.proxy_owner`：代理服主级别（默认 `False`）。

实际生效级别还会参考 `chat_config.json` 的 `default_permission_level`、`op_maps_to_admin`、`permission_overrides`（取较高档）。
...
- 银行 / 领地 / 传送工具需弧光核心 ≥ **0.8.12**；天眼扩展事件同版本。
- 若 AI 请求失败，插件会通过玩家聊天或服务器日志输出错误信息，方便排查。

### 更新日志

- **2.0.0**：大版本。AI 权限正式拆为三档（助手 / 管理员 / 代理服主）；对接弧光核心银行、领地、传送 API（`mc_economy` / `mc_land` / `mc_arc_tp`，管理员及以上）；Hub 对话附带 `permission_level`；`chat_config` 新增 `default_permission_level` / `op_maps_to_admin` / `permission_overrides`。
